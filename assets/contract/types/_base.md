...
fields:
  ...
  review:     { type: enum,     required: false, values: [needed, legacy-unknown], note: "independent of execution status; remove after explicit review; legacy needs-review has unknown prior state" }
...
Trailing annotations are opaque; repair only the diagnosed line and preserve its
neighbors. Checklist continuations follow a recognized step; malformed text ends
attachment. These rules change no concrete type's fields or section requirements.

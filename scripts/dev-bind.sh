...
#      bin/arbiter.js with ARBITER_DATA defaulted to this repo's arbiter-data/
#   2. appends a fenced ARBITER_DATA export to ~/.bash_profile and ~/.bashrc
#
...
export ARBITER_DATA="\${ARBITER_DATA:-$REPO_DIR/arbiter-data}"
...
EOF
    ...
    for f in $PROFILES; do
      strip_block "$f"
      printf '\n%s\n' "$MARK_START" >> "$f"
      printf 'export ARBITER_DATA="%s/arbiter-data"\n' "$REPO_DIR" >> "$f"
      printf '%s\n' "$MARK_END" >> "$f"
    done
    ...
    echo "installed: $WRAPPER (data: $REPO_DIR/arbiter-data)"

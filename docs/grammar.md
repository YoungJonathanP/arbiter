...
*v0.4 · Normative spec for the parts of arbiter-data files that tools parse, validate, and rewrite. Companion to [`arbiter-data/PROTOCOL.md`](../arbiter-data/PROTOCOL.md) (the agent-facing contract) — where the two disagree, this document wins for machines. Everything **not** matched by a production here is opaque prose: tools MUST preserve it byte-for-byte and MUST NOT derive meaning from it.*
...
rel-path    ::= (item-dir | "archive/" year "-q" ("1".."4") "/" item-dir) "/" path-tail
...
Canonical form of a `ref-list` is a YAML flow sequence: `related: [q3-deploy-pipeline-2026q3]`.
...
checklist    ::= "## Checklist" eol step+
artifacts    ::= "## Artifacts" eol link-entry+
evidence     ::= "## Evidence" eol link-entry+
detail-docs  ::= "## Detail docs" eol docs-entry+
